...
export default async function handler(req, res) {
  ...
  if (mode === "shiller" || mode === "impact") {
    ...
    if (window === "custom" && req.body.customStart && req.body.customEnd) {
      start_date = req.body.customStart;
      end_date = req.body.customEnd;
    } else {
      let daysAgo = 1;
  ...
  try {
    ...
    try {
      ...
      await db.collection("search_history").insertOne({
        ...
        mode,
        window: mode === "shiller" || mode === "impact" ? window : null,
        includeTicker,
        job_id: data.job_id,
        created_at: new Date(),
      });
