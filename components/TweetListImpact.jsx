...
async function fetchBatchPrices(baseTweets, contractAddress) {
  ...
  for (const tweet of baseTweets) {
    const tweetId = String(tweet.tweetId || tweet.tweet_id || tweet.id);
    if (!tweetId) continue;
    for (const { key } of timeOffsets) {
      keys.push({ tweet_id: tweetId, key });
    }
  }
  ...
  if (!data.prices || typeof data.prices !== "object") {
    console.warn("⚠️ Invalid prices format from API:", data);
    return {};
  }
...
export default function TweetListImpact({
  ...
  useEffect(() => {
    ...
    for (const tweet of realTweets) {
      ...
      for (const { key: keyForThisPrice, minutes } of timeOffsets) {
        const priceKey = `${tweetId}-${keyForThisPrice}`;
        if (!(priceKey in batch) || batch[priceKey] === null) {
            ...
            if (data.job_id) {
              ...
              setTweetsWithPrices((prev) =>
                prev.map((t) => {
                  ...
                  if (`${id}-${keyForThisPrice}` === priceKey) {
                    return {
                      ...t,
                      [keyForThisPrice]: result ?? null,
                    };
                  }
                  return t;
                })
    ...
    setTweetsWithPrices((prevTweets) =>
      prevTweets.map((tweet) => {
        const tweetId = tweet.tweetId || tweet.tweet_id || tweet.id;
        const updated = { ...tweet };
        for (const { key } of timeOffsets) {
          const priceKey = `${tweetId}-${key}`;
          if (priceMap[priceKey] != null) {
            updated[key] = priceMap[priceKey];
          }
        }
        return updated;
      })
    ...
    const finalUpdated = realTweets.map((tweet) => {
      ...
      return {
        ...tweet,
        price: batch[`${tweetId}-price`] ?? null,
        price_15m: batch[`${tweetId}-price_15m`] ?? null,
        price_30m: batch[`${tweetId}-price_30m`] ?? null,
        price_1h: batch[`${tweetId}-price_1h`] ?? null,
        price_3h: batch[`${tweetId}-price_3h`] ?? null,
      };
    });
  ...
  const paged = useMemo(() => {
    return enrichedTweets.slice((page - 1) * pageSize, page * pageSize).map((tweet) => {
      const tweetId = tweet.tweetId || tweet.tweet_id || tweet.id;
      return {
        ...tweet,
...
if (loading) {
  ...
  if (!enrichedTweets || enrichedTweets.length === 0) {
  ...
  if (viewMode === "list") {
    return (
      <>
        ...
        <ImpactfulCallersCard tweets={enrichedTweets} />
        ...
        <button
          onClick={() =>
            setPage((p) =>
              p < Math.ceil(enrichedTweets.length / pageSize) ? p + 1 : p
            )
          }
          disabled={page >= Math.ceil(enrichedTweets.length / pageSize)}
          className="px-4 py-1 rounded bg-zinc-700 text-white disabled:opacity-50"
        >
          Next
        </button>
